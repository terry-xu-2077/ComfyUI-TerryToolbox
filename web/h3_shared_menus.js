...
function linksProp(mode) {
  return mode === "timeline" ? TIMELINE_LINKS : PROMPT_LINKS;
}
...
function graphNode(node, id) {
  return node?.graph?.getNodeById?.(Number(id)) || app.graph?.getNodeById?.(Number(id)) || null;
}
...
function isDirectUsed(node, mode, asset) {
  return new RegExp(`<${asset.label.replace(/\s+/g, "\\s+")}>`, "i").test(promptText(node, mode));
}
...
function makeChip(mode, raw, label, kind = "") {
  ...
  if (mode === "timeline") {
    ...
    const cls = kind === "subject" ? "terry-h3-subject-asset-chip terry-h3-strong" : kind ? "terry-h3-media-chip" : "";
    el.className = `terry-h3-chip ${cls}`.trim();
  }
...
function closeMenu(controller) {
  ...
  controller.menuType = null;
  controller.commandState = null;
}

function currentFilter(node) {
  return String(node?.properties?.[FILTER_PROP] || "subject");
}
...
function openAssetMenu(controller) {
  ...
  for (const asset of visible) {
    ...
    const parts = definitionParts(node, mode, asset);
    meta.textContent = filter === "defined" ? parts.join(" · ") : `${asset.kind === "picture" ? "图片" : asset.kind === "video" ? "视频" : "音频"}资产 · ${asset.displayLabel}`;
    ...
    if (filter === "subject") {
      if (asset.kind !== "audio") {
        ...
        button.addEventListener("pointerdown", (event) => {
          event.preventDefault(); event.stopPropagation();
          const number = subjectFor(node, mode, asset);
          insertAt(editor, hit.range, makeChip(mode, `<Subject ${number}>`, `主体 ${number}`, "subject"), controller.onChange);
          closeMenu(controller);
        });
      ...
      button.addEventListener("pointerdown", (event) => {
        ...
        if (asset.preview && asset.kind !== "audio") {
          const img = document.createElement("img"); img.src = asset.preview; img.alt = ""; chip.prepend(img);
        }
        insertAt(editor, hit.range, chip, controller.onChange);
        closeMenu(controller);
      });
      actions.append(button);
    }
...
function nextSpeaker(node, mode) {
  let max = 0;
  for (const match of promptText(node, mode).matchAll(/\(S(\d+)\)/gi)) max = Math.max(max, Number(match[1]) || 0);
  return max + 1;
}
...
function chooseCommand(controller, state, command) {
  ...
  insertAt(controller.editor, state.range, makeChip(controller.mode, raw, label, kind), controller.onChange);
  closeMenu(controller);
}
...
function renderCommandMenu(controller, state) {
  ...
  if (state.category && !search) {
    ...
    back.addEventListener("pointerdown", (event) => { event.preventDefault(); state.category = null; state.active = 0; renderCommandMenu(controller, state); });
    title.append(back);
  }
  const bold = document.createElement("b"); bold.textContent = search ? "搜索 H3 语法" : state.category ? category(state.category).label : "H3 语法";
  title.append(bold);
  ...
  if (!state.category && !search) {
    state.options = CATEGORY_META
      .map((meta) => ({ type: "category", meta, count: state.list.filter((item) => item.category === meta.id).length }))
      .filter((item) => item.count > 0);
  } else {
    const rows = state.category && !search ? state.list.filter((item) => item.category === state.category) : state.list;
...
function refreshOpenMenu(controller) {
  if (controller.menuType === "asset") openAssetMenu(controller);
  else if (controller.menuType === "command") openCommandMenu(controller);
}
...
function handleCommandKey(controller, event) {
  ...
  if (["ArrowRight", "Enter", "Tab"].includes(event.key)) {
    ...
    if (option.type === "category") { state.category = option.meta.id; state.active = 0; renderCommandMenu(controller, state); }
...
export function attachH3Menus({ node, editor, mode = "prompt", onChange = null }) {
  ...
  const onBlur = () => setTimeout(() => {
    if (!controller.menu?.matches?.(":hover")) closeMenu(controller);
  }, 120);
